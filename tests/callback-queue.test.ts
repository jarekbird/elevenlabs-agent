...
import { CallbackQueueService, type CallbackTask } from '../src/services/callback-queue';
...
describe('CallbackQueueService', () => {
  ...
  describe('Race Condition Safety', () => {
    ...
    it('maintains correct session mapping during concurrent updates', async () => {
      ...
      expect((final?.sessionPayload as { sessionId: string })?.sessionId).toBe('session-123');
    });
  });
});
