  ...
  describe('getSignedUrl', () => {
    it('requests signed URL for default agent', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.elevenlabs.io/v1/convai/signed-url',
    ...
    it('requests signed URL for specific agent', async () => {
      ...
      expect(result.signedUrl).toBe('wss://example.com/signed-url');
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.elevenlabs.io/v1/convai/agents/agent-123/signed-url',
        expect.any(Object)
      );
    });
    ...
    it('handles network errors', async () => {
      ...
      const client = new ElevenLabsApiClient();
      await expect(client.getSignedUrl()).rejects.toThrow('Failed to get signed URL');
    });
    ...
    it('handles timeout errors', async () => {
      ...
      const client = new ElevenLabsApiClient();
      await expect(client.getSignedUrl()).rejects.toThrow('Request to ElevenLabs API timed out');
    });

    it('throws error when API key is not configured', async () => {
      delete process.env.ELEVENLABS_API_KEY;
      const client = new ElevenLabsApiClient();
      await expect(client.getSignedUrl()).rejects.toThrow('ELEVENLABS_API_KEY is required');
    });
    ...
    it('handles invalid response structure', async () => {
      ...
      const client = new ElevenLabsApiClient();
      await expect(client.getSignedUrl()).rejects.toThrow('Invalid response from ElevenLabs API');
    });
