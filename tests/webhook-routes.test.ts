...
describe("Webhook Routes", () => {
  ...
  describe("GET /signed-url", () => {
    ...
    it("should return webhook URL when ELEVENLABS_API_KEY is configured", async () => {
      process.env.ELEVENLABS_AGENT_URL = "http://test:3004";

      // Mock ElevenLabs API response - use mockResolvedValueOnce to ensure it's called
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          signed_url:
            "wss://api.elevenlabs.io/v1/convai/conversation?agent_id=test&token=abc123",
          expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
        }),
      } as Response);

      // Create new server instance after setting env vars
      server = new Server();
      await server.start();
    ...
    it("should return 500 when ELEVENLABS_API_KEY is not configured", async () => {
      ...
      // Create new server instance after deleting env var
      server = new Server();
      await server.start();
      ...
      expect(response.body.error).toContain("ELEVENLABS_API_KEY");
    });
  });
  ...
  describe("POST /agent-tools", () => {
    ...
    it("should accept valid agent tool request", async () => {
      ...
      // 1. POST to create agent conversation -> returns conversationId
      // 2. GET to fetch the created conversation -> returns full conversation
      // 3. POST to add message -> returns success
      // 4. POST to cursor-runner -> returns requestId
      mockFetch
      ...
      const response = await request(server.app).post("/agent-tools").send({
        agent_id: "test-agent",
  ...
  describe("POST /callback", () => {
    ...
    it("should accept valid callback payload", async () => {
      ...
      const response = await request(server.app).post("/callback").send({
        success: true,
        requestId: "test-request-id",
  ...
  describe("Callback Queue Behavior and Race Conditions", () => {
    ...
    it("should handle multiple callbacks for same conversation", async () => {
      ...
      const callback1 = await request(server.app).post("/callback").send({
        success: true,
      ...
      const callback2 = await request(server.app).post("/callback").send({
        success: true,
    ...
    it("should handle rapid sequential callbacks", async () => {
      ...
      const callbacks = await Promise.all([
        request(server.app).post("/callback").send({
          success: true,
          requestId: "req-1",
          conversationId: "conv-1",
          timestamp: new Date().toISOString(),
  ...
  describe("Signed URL Route", () => {
    ...
    it("should include agent_id in response when provided", async () => {
      ...
      // Mock ElevenLabs API response - must be set before creating server
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          signed_url:
            "wss://api.elevenlabs.io/v1/convai/conversation?agent_id=test-agent-123&token=abc123",
          expires_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
        }),
      } as Response);

      // Create new server instance after setting env vars and mock
      server = new Server();
      await server.start();
  ...
  describe("Webhook Handling and Immediate Responses", () => {
    ...
    it("should respond immediately to agent-tools request", async () => {
      ...
      expect(response.status).toBe(200);
      // Should respond quickly (within 500ms) even though cursor-runner takes longer
      // Increased timeout to account for async operations
    ...
    it("should handle cursor-runner errors gracefully", async () => {
      ...
      // 2. GET to fetch the created conversation -> returns full conversation
      // 3. POST to add message -> returns success
      // 4. POST to cursor-runner -> fails with network error
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
        ...
        .mockImplementationOnce((url: string | URL | Request) => {
          ...
          if (
            urlStr.includes("cursor-runner") ||
            urlStr.includes("/cursor/execute")
          ) {
